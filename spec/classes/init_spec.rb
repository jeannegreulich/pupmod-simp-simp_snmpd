    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context "install manifest with default parameters" do
          ...
          it { is_expected.to create_file('/etc/snmp/snmptrapd.d').with({
            :ensure => 'absent'
          })}
        end
        ...
        context "install/users with default parameters" do
          it { is_expected.to contain_snmp__snmpv3_user('snmp_ro').with({
            :authtype => 'SHA',
            :privtype => 'AES'
          })}
          it { is_expected.to contain_snmp__snmpv3_user('snmp_rw').with({
            :authtype => 'SHA',
            :privtype => 'AES'
          })}
        end
        ...
        context "with some parameters set" do
          ...
          it { is_expected.to contain_file('/etc/snmp/snmpd.d').with({
            :ensure => 'directory'
          })}
          it { is_expected.to contain_class('snmp').with({
             :snmpd_config   => [
