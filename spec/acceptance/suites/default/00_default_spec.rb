  ...
  context 'with default setting on snmpd agent and client installed' do
    hosts.each do |node|
      ...
      it 'should not work for undefined users' do
        result = on(node, '/usr/bin/snmpwalk -u snmp_rx -X KeepItSafe -A KeepItSecret localhost sysLocation.0')
        expect(result.stdout).to include("Unknown user name (Sub-id not found: (top)")
      end
    end
  end
end
